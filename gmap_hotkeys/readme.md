...
Gmap hotkeys is the baseline accessiblity addon for the Unseen Godot project, which is to leverage the AccessKit integration to make the Godot Editor easier for devs using screen readers.
...
## Hotkeys:
All hotkeys at the moment need "gmap_ctrl" also held down, which is CTRL by default, but can be changed inside of plugin.gd easily. Can edit what keys are used as well in same place.		
### Goto Hotkeys:
...
## UI Changes:
1. VBOX and HBOX containers get a script that makes it whenever they are focused, each item is hard set so each item is neighbor of next sibling in list.
1.1 registers these scripts as GmVBoxContainer and GmHBoxContainer so can be selected as nodes in your own project.
